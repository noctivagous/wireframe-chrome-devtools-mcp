...
const gridOverlayLayerSchema: zod.ZodTypeAny = zod.object({
  ...
  offset: lengthSchema.optional().describe('Optional overlay offset.'),
...
const layoutParametricGridSchema: zod.ZodTypeAny = zod.object({
  ...
  gap: lengthSchema.optional().describe('Grid gap applied to all rows by default.'),
  overlay: zod
...
function renderContentItem(item: ContentItem, prefix: string): {
  html: string;
  css: string;
  behaviors: BehaviorSpec[];
  ...
  if (typeof item === 'object' && item && 'composition' in item && item.composition) {
    const nested = renderComposition(item.composition as any, prefix);
    return {
...
function buildGridRowHtml(
  ...
  row.items.forEach((item: ContentItem, itemIndex: number) => {
    const rendered = renderContentItem(item, prefix);
  ...
  const rowCss = [
    `grid-template-columns: repeat(${Math.max(totalUnits, 1)}, minmax(0, ${unit}));`,
  ];
...
function buildGridOverlayHtml(
  ...
  const offsetUnits = parseUnitOffset(layer.offset);
  const totalUnits =
    spans.reduce((acc: number, span: number) => acc + (Number.isFinite(span) ? span : 1), 0) +
    (offsetUnits ? Math.ceil(offsetUnits) : 0);
  const templateUnits = Math.max(totalUnits, baseUnits);
  ...
  let html = `<div class="${prefix}-grid-overlay" data-le-overlay="${layerIndex}" style="z-index:${zIndex};">`;
  if (offsetUnits && offsetUnits > 0) {
    const spanValue = Math.max(1, Math.ceil(offsetUnits));
    html += `<div class="${prefix}-grid-spacer" style="grid-column: span ${spanValue};"></div>`;
  }
...
function buildGridLayout(params: LayoutGridParams, prefix: string): RenderedComposition {
  ...
  if (params.overlay && params.overlay.length > 0) {
    params.overlay.forEach((layer: GridOverlayLayer, index: number) => {
      ...
      } = buildGridOverlayHtml(layer, prefix, index, unit, maxRowUnits);
  ...
  const css = `
    ...
    .${prefix}-grid-base{display:flex;flex-direction:column;gap:${baseGap ?? '0.5rem'};}
    .${prefix}-grid-row{display:grid;align-items:stretch;}
    .${prefix}-grid-item,.${prefix}-grid-overlay-item{display:flex;align-items:center;justify-content:center;}
    .${prefix}-grid-overlay{position:absolute;inset:0;display:grid;pointer-events:none;}
    .${prefix}-grid-overlay-item{pointer-events:auto;}
    ${rowStyles.join('\n')}
    ${overlayStyles.join('\n')}
  `;

  return {html, css: `${css}\n${nestedCss.join('\n')}`, behaviors: nestedBehaviors};
}
...
function buildViewerComponent(params: ComponentViewerParams, prefix: string): RenderedComposition {
  ...
  const panelsHtml = params.items
    .map((item: any, index: number) => {
      ...
      if (typeof item.content === 'string') {
        ...
        const nested = renderComposition((item.content as any).composition, prefix);
...
function renderComposition(composition: any, prefix: string): RenderedComposition {
  if (!composition || typeof composition !== 'object') {
    return {html: '', css: '', behaviors: []};
  }
  if (composition.type === 'layout_parametric_grid') {
    const result = buildGridLayout(composition.params, prefix);
    const behaviors = [...(composition.behaviors ?? []), ...result.behaviors];
    return {html: result.html, css: result.css, behaviors};
  }
  if (composition.type === 'layout_parametric_stack') {
    ...
    const result = buildStackLayout(composition.params, prefix, {enableResize});
    const updatedHtml = enableResize
      ...
      : result.html;
    return {html: updatedHtml, css: result.css, behaviors: [...behaviors, ...result.behaviors]};
  }
  if (composition.type === 'component_parametric_viewer') {
    const result = buildViewerComponent(composition.params, prefix);
    const behaviors = [...(composition.behaviors ?? []), ...result.behaviors];
    return {html: result.html, css: result.css, behaviors};
  }
  return {html: '', css: '', behaviors: []};
...
export const layoutLiveEditingRecipeCatalog = defineTool({
  name: 'layout_live_editing_recipe_catalog',
  description: 'List layout_live_editing recipes and summarize their parameter schemas.',
  annotations: {
...
export const layoutLiveEditing = defineTool({
  ...
  handler: async (request, response, context) => {
    ...
    const rendered = renderComposition(resolvedComposition, prefix);
